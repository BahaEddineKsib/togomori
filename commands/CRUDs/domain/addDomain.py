...
class AddDomain:
	...
	def execute(IN):
		...
		if("UserNeedsHelp" in [ad, domain, for_sure] or (not domain)):
			AddDomain.help()
			...
			dmn = Domain(workshop_id=w_id, domain_text=domain)
			print("\ndomain:")
			dmn.display()
			...
			elif(result == "MainDomainExist"):
				print("❌  Main Domain already exist. add < --sub >")
