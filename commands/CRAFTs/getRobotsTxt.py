...
from get_assets.getRobotsTxtByDomain	import GetRobotsTxtByDomain 
...
class GetRobotsTxt:
	...
	def execute(IN):
		...
		IN	     = c.short_command(IN,"robots")
		cmnd	     = c.option("robots" ,False, False,IN)
		...
		if("UserNeedsHelp" in [ cmnd, domain, no_save, workshop, all_domains, domains_list]):
			...
			if all_domains:
				if workshop and Workshop.exist(workshop):
					...
					for d in dmns:
						if d.robots_file =="":
							result = GetRobotsTxtByDomain(workshop,d.domain,no_save)
				else:
					pp("❌ Set a Workshop or specify a workshop with [-w <workshop id>]")
					return "NoWorkshopSetted"
			elif domains_list:
				for d in domains_list:
					if c.segmentUrl(d)['domain'] != "NoDomain":
						result = GetRobotsTxtByDomain(workshop, d, no_save)
					else:
						pp(d+" is not a good domain")
			else:
				domain   = c.segmentUrl(domain)['domain'] if domain else TopG.CURRENT_DOMAIN

				result = GetRobotsTxtByDomain(workshop,domain,no_save)
				'''
				match result:
	...
	-d <domain>		  Y/N		insert a domain to get its robots.txt
						required when there is no domain setted

	-a			  NO		get the robots.txt of all the domains 
						in the workshop that don't have yet

	-l			  NO		insert a list of domains to get their robots.txt

	-w <workshop>		  Y/N		select a workshop to add the domain in it
						required when there is no workshop setted

	-no			  NO		do NOT save the robots.txt we got.
		""")
